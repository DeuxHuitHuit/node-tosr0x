...
const p = function (handler) {
	return new Promise(handler);
};

const ptime = function (handler, delay, timeout) {
	return p(function (res, rej) {
		const t = setTimeout(function () {
			call(timeout);
			rej(new Error('Tosr0x timeout'));
		}, delay || 0);
		const newResolve = function () {
			clearTimeout(t);
			return res.apply(this, arguments);
		};
		const newReject = function () {
			clearTimeout(t);
...
const resolved = function (err, rej, callback) {
	if (!!err) {
		call(callback, err);
		rej(err);
	}
...
T.device = {
	vendorId: parseInt('0x0403', 16),
	productId: parseInt('0x6001', 16),
	moduleId: 15
};
	...
	return p(function (res, rej) {
		T.portScan(function (ports) {
			...
			ports.forEach(function (port) {
				if (options.debug) {
					console.log(
						'* Found device on port %s, device made by %s (%s), product %s (%s)',
						port.comName,
						port.manufacturer,
						port.vendorId,
						port.serialNumber,
				...
				if (parseInt(port.vendorId, 16) === device.vendorId &&
					parseInt(port.productId, 16) === device.productId) {
					const t = new T(port.comName, options);
			...
			if (ports.length === 0) {
				const err = new Error('No port found!');
...
const bufferIsNullTerminated = function (buffer) {
	return buffer.indexOf('\n') !== -1;
};

const dataListener = function (t) {
...
const errorListener = function (t) {
...
T.prototype.open = function (callback) {
	...
	return p(function open(res, rej) {
		var s = new SerialPort(t._uri, opts);
		log(t, 'Opening port...');
		s.open(function (err) {
...
T.prototype.send = function (cmd, callback) {
	...
	return ptime(function send(res, rej) {
		log(t, '------------------------------');
		log(t, 'Sending ' + cmd);
		s.write(cmd, 'ascii', function (err, bytesWritten) {
			if (resolved(err, rej, callback)) {
...
T.prototype.switch = function (relay, on, callback) {
	...
	const cmd = !!on ? T.commands.on : T.commands.off;
	return this.send(String.fromCharCode(cmd + relay), function (err, ret) {
		...
		if (relay === 0) { // all relays
			_.forOwn(this._state, function (val, key) {
				this._state[key] = !!on;
			}, this);
		} else {
			this._state[relay] = !!on;
		}
	});
};
...
T.prototype.toggle = function (relay, callback) {
	return this.switch(relay, !this._state[relay], callback);
};

T.prototype.states = T.prototype.refreshStates = function (callback) {
	const t = this;
	const promise = ptime(function version(res, rej) {
		t.receive(function (data) {
			const binStates = data[0];
			const relayCount = t._options.relayCount;
			for (var x = 0; x < relayCount; x++) {
				var mask = (1 << x);
				t._state[x + 1] = (binStates & mask) === mask;
			}
			call(callback, t._state);
			res({states: t._state, ctl: t});
		});
	}, this._options.timeout, function () { t.closeImmediate(); });
...
T.prototype.voltage = function (callback) {
	const t = this;
	const promise = ptime(function voltage(res, rej) {
		t.receive(function (data) {
...
T.prototype.temperature = function (callback) {
	const t = this;
	const promise = ptime(function temperature(res, rej) {
		t.receive(function (data) {
			call(callback, data);
